...
export class DLogToRec709Converter {
   ...
   */
  private buildFilter(
    method: 'simple' | 'advanced' = 'advanced',
    dlogType: 'dlog' | 'dlogm' = 'dlogm',
    lutPath?: string,
  ): string {
  ...
  private buildAdvancedFilter(dlogType: 'dlog' | 'dlogm' = 'dlogm'): string {
    ...
    const dlogFilters: Record<string, string> = {
      ...
      dlogm:
        'zscale=transfer=linear,primaries=bt709:transfer=bt709,zscale=transfer=bt709:primaries=bt709:matrix=bt709',
    }
  ...
  async convert(
    ...
    return new Promise((resolve, reject) => {
      ...
      const args: string[] = [
        ...
        inputPath, // 输入文件
        '-vf',
        filterStr + ',scale=iw:ih,format=yuv420p', // 视频滤镜：色彩转换、缩放、像素格式
        '-c:v',
        'libx264', // 使用 H.264 编码
        '-preset',
        preset, // 编码预设
        ...
        '192k', // 音频比特率
        '-movflags',
        '+faststart', // 启用 faststart，便于流媒体播放
  ...
  async batchConvert(
    ...
    const processFile = async (): Promise<BatchResult | null> => {
      ...
      // 执行转换并处理结果
      const batchResult = await this.convert(input, output, fileConfig)
        .then(
          () =>
            ({
              input,
