  ...
  return (
    <div className={`p-4 rounded-lg ${convertResult.success ? 'bg-green-50' : 'bg-yellow-50'}`}>
      <p className={`text-sm ${convertResult.success ? 'text-green-800' : 'text-yellow-800'}`}>
        转换完成: 共 {convertResult.summary.total} 个文件， 成功 {convertResult.summary.success}{' '}
        个， 失败 {convertResult.summary.failed} 个
      </p>
      {convertResult.error && (
        <p className="text-sm text-red-600 mt-2">错误信息: {convertResult.error}</p>
      )}
